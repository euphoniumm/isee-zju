...
def main():
    ...
    save_dir = home_dir/'..'/'data'/test_split/'results2'
    ...
    n_particles = 600            # Number of particles used in particle filter
    ...
    for idx in range(1, n_imgs, step):
        ...
        k = 20
        max_idx = weights.argsort()[-k:][::-1]
        weights_sum = 0
        weights_k = np.zeros(k)
        for i in range(k):
        ...
        particles = resample_step(particles, weights, rsp_sigmas=[2, 2, 0.015, 0.015], topk=k)
